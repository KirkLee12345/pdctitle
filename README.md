...
纯服务端 Minecraft **Fabric 26.2** 称号模组，原版客户端即可加入、无需任何玩家安装 mod。所有指令以 `/plttitle`（别名 `/plt`）为根，避开原版已占用的 `/title`。
...
- 数据存 `config/plttitle/definitions.json`（称号池）+ `players.json`（归属/佩戴），原子写盘；
...
```bash
./gradlew build     # 产物：build/libs/plttitle-<version>.jar
```
...
3. 启动服务器，自动生成 `config/plttitle/definitions.json` 与 `players.json`；
4. 游戏内用 `/plttitle` 操作（OP 可用池/归属管理指令）。

## 快速上手

```
/plttitle add legend &b[至尊] 开服元老纪念称号   # OP：建池条目（空格后为可选描述，一条搞定）
/plttitle desc legend 开服元老纪念称号          # OP：单独补/改描述（可选）
/plttitle grant KirkLee123 legend              # OP：授权（可反复授权给不同玩家）
/plttitle my                                   # 玩家：查看自己的称号
/plttitle wear legend                          # 玩家：佩戴
/plttitle wear none                            # 玩家：卸下
/plttitle remove legend                        # OP：删除定义（所有玩家该称号随之消失）
```
