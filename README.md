...
> 只想先跑起来？看这一节就够；单页精简版见 [`docs/05-快速上手.md`](docs/05-快速上手.md)。

**先记住 3 个概念**：① **称号池**（OP 建，全服共用模板：id + 显示文案 + 描述）→ ② **授权**（OP 把池里的称号发给玩家，可发给很多人）→ ③ **佩戴**（玩家自己在已授权列表里选 1 个戴上）。全程按 UUID 记账，玩家改名不掉授权；OP 删掉池里的定义，所有人的该称号级联消失。

1. **装上**：Fabric 服务器（MC 26.2 / Loader ≥ 0.19.3 / Java 25），把 jar 放进 `mods/` 后重启，自动生成 `config/pdctitle/`；
2. **OP 建称号 + 授权**：
   ```
...
| `docs/05-快速上手.md` | **一页精简版**：3 步跑通、全指令速查、引号/颜色码规则、显示开关、常见问题 |
